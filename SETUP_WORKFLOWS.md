# GitHub Workflows Setup Guide

GitHub OAuth 앱은 workflow 파일을 푸시할 권한이 없어서, GitHub 웹사이트에서 직접 생성해야 합니다.

## 🚀 Quick Setup (5분)

### Step 1: API Key Secret 설정

1. GitHub 리포지토리로 이동: https://github.com/Maverick-jkp/jakes-tech-insights
2. **Settings** → **Secrets and variables** → **Actions** 클릭
3. **New repository secret** 클릭
4. 다음 입력:
   - **Name**: `ANTHROPIC_API_KEY`
   - **Secret**: 회사 Claude API 키 입력
5. **Add secret** 클릭

### Step 2: Daily Content Workflow 생성

1. GitHub 리포지토리의 **Actions** 탭 클릭
2. **New workflow** 클릭
3. **set up a workflow yourself** 클릭
4. 파일 이름을 `daily-content.yml`로 변경
5. 아래 내용 전체 복사해서 붙여넣기:

```yaml
name: Daily Content Generation

on:
  schedule:
    # Run every day at 9 AM UTC (6 PM KST)
    - cron: '0 9 * * *'
  workflow_dispatch:
    inputs:
      count:
        description: 'Number of posts to generate'
        required: false
        default: '3'
      skip_review:
        description: 'Skip AI review step'
        required: false
        default: 'false'

jobs:
  generate-content:
    runs-on: ubuntu-latest

    steps:
      - name: Checkout repository
        uses: actions/checkout@v4
        with:
          token: ${{ secrets.GITHUB_TOKEN }}

      - name: Set up Python
        uses: actions/setup-python@v5
        with:
          python-version: '3.11'

      - name: Install dependencies
        run: |
          python -m pip install --upgrade pip
          pip install anthropic

      - name: Generate content
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
        run: |
          COUNT="${{ github.event.inputs.count || '3' }}"
          python scripts/generate_posts.py --count "$COUNT"

      - name: Run quality gate
        id: quality_gate
        run: |
          python scripts/quality_gate.py
          echo "quality_gate_passed=$?" >> $GITHUB_OUTPUT

      - name: AI Review
        if: github.event.inputs.skip_review != 'true'
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
        run: |
          python scripts/ai_reviewer.py

      - name: Upload quality reports
        uses: actions/upload-artifact@v4
        with:
          name: quality-reports
          path: |
            quality_report.json
            ai_review_report.json
            generated_files.json
          retention-days: 30

      - name: Create Pull Request
        if: success()
        uses: peter-evans/create-pull-request@v6
        with:
          token: ${{ secrets.GITHUB_TOKEN }}
          commit-message: |
            🤖 Auto-generated content

            Generated ${{ github.event.inputs.count || '3' }} blog posts

            Quality Gate: ✅ PASSED
            AI Review: See artifacts for details
          branch: auto-content-${{ github.run_number }}
          delete-branch: true
          title: '🤖 Daily Content: ${{ github.event.inputs.count || 3 }} new posts'
          body: |
            ## 🤖 Automated Content Generation

            This PR contains AI-generated blog content that has passed quality checks.

            ### 📊 Generation Details
            - **Posts generated**: ${{ github.event.inputs.count || '3' }}
            - **Workflow run**: [${{ github.run_number }}](${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }})
            - **Quality gate**: ✅ PASSED

            ### 📋 Quality Reports
            Download the artifacts from the workflow run to see:
            - `quality_report.json` - Detailed quality metrics
            - `ai_review_report.json` - AI self-review results
            - `generated_files.json` - List of generated files

            ### ✅ Checklist for Review
            - [ ] Review generated content for accuracy
            - [ ] Check AI review recommendations
            - [ ] Verify quality gate metrics
            - [ ] Test locally if needed

            ### 🚀 Next Steps
            1. Review the generated content
            2. Make any necessary edits
            3. Merge to trigger Cloudflare Pages deployment

            ---
            *Generated by GitHub Actions*
          labels: |
            automated
            content
            needs-review

      - name: Cleanup on failure
        if: failure()
        run: |
          # Clean up stuck topics if workflow fails
          python scripts/topic_queue.py cleanup 24

      - name: Notify on failure
        if: failure()
        run: |
          echo "::warning::Content generation workflow failed. Check logs and quality reports."
          echo "::warning::Stuck topics have been cleaned up and returned to queue."
```

6. **Commit changes** 클릭 (커밋 메시지: `feat: Add daily content generation workflow`)

### Step 3: 테스트 실행

1. **Actions** 탭으로 이동
2. 좌측에서 **Daily Content Generation** 선택
3. **Run workflow** 드롭다운 클릭
4. 옵션 설정:
   - **Number of posts**: `1` (테스트용)
   - **Skip AI review**: `false`
5. **Run workflow** 버튼 클릭

### Step 4: 결과 확인

- Workflow 실행이 완료되면 (약 3-5분)
- 자동으로 PR이 생성됨
- **Pull requests** 탭에서 확인
- Quality reports는 Artifacts에서 다운로드 가능

## 📋 Workflow 기능

### 자동 실행
- 매일 오전 9시 UTC (오후 6시 KST) 자동 실행
- 기본값: 3개 포스트 생성

### 수동 실행
- Actions 탭에서 언제든 수동 실행 가능
- 포스트 개수 커스터마이징 가능
- AI 리뷰 스킵 옵션

### 파이프라인
```
1. 토픽 큐에서 예약
2. Claude API로 Draft 생성
3. Editor Agent가 검토
4. Quality Gate 통과 확인
5. AI Reviewer 평가
6. Reports 생성
7. PR 자동 생성
8. 실패 시 자동 클린업
```

## ✅ 완료 체크리스트

- [ ] ANTHROPIC_API_KEY Secret 설정
- [ ] daily-content.yml 파일 생성
- [ ] 테스트 workflow 실행 (1개 포스트)
- [ ] PR 자동 생성 확인
- [ ] Quality reports 다운로드 및 확인
- [ ] 생성된 콘텐츠 품질 체크

## 🔧 Troubleshooting

### Workflow가 실행되지 않는 경우
- Actions 탭이 활성화되어 있는지 확인
- Repository Settings → Actions → General에서 권한 확인

### API Key 에러
- Secret 이름이 정확히 `ANTHROPIC_API_KEY`인지 확인
- API 키가 유효한지 확인

### PR 생성 실패
- `GITHUB_TOKEN` 권한 확인
- Repository Settings → Actions → General → Workflow permissions → "Read and write permissions" 체크

## 📞 다음 단계

Workflow 설정이 완료되면:

1. **로컬에서 테스트**:
   ```bash
   # Generate 1 post locally
   export ANTHROPIC_API_KEY='your-key'
   python scripts/generate_posts.py --count 1

   # Run quality gate
   python scripts/quality_gate.py

   # Run AI review
   python scripts/ai_reviewer.py
   ```

2. **Queue 관리**:
   ```bash
   # View stats
   python scripts/topic_queue.py stats

   # Add new topics
   python scripts/topic_queue.py add "New keyword" tech en 8
   ```

3. **점진적 확대**:
   - Week 1: 1개/일 (수동 체크)
   - Week 2-4: 3개/일
   - Month 2+: 9개/일 (자동화)

---

**설정 완료 후 이 파일은 삭제해도 됩니다.**
