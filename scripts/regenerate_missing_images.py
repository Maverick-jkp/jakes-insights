#!/usr/bin/env python3
"""
Regenerate missing images for existing posts

This script:
1. Finds posts with missing image files
2. Extracts keywords from post metadata
3. Downloads images from Unsplash using the fixed filename logic
4. Updates used_images.json tracking
"""
...
def find_posts_with_missing_images() -> List[Dict]:
    ...
    for post_path in Path('content').rglob('*.md'):
        ...
        if not full_path.exists():
            # Extract keyword from tags (parse JSON array)
...
def main():
    ...
    for i, post in enumerate(missing_posts, 1):
        ...
        if not post['keyword']:
            print(f"  ⚠️  No keyword found, skipping")
            failed_count += 1
            continue
